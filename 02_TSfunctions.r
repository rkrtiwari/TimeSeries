...
ccf(tas.h[,1], tas.h[,2], lag = 10, na.action=na.pass) 
lag2.plot (tas.h[,1], tas.h[,2], 10)


############################### calculate lags and differences in data #######################
# lagging in time series > align time series for comparison and 
# shift observations in time
...
new=cbind(ndvi,lag_xts)  # combine columns for comaprison

# differencing series
ndvi -lag(ndvi) # calculate first diff using lag and subtraction
